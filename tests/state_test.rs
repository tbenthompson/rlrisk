...
fn test_init_state(n_players: usize, n_territories: usize) {
    let game = risk_ext::start_game(n_players, n_territories, 1, 0.0, 1, 1, 0);
...
#[test]
fn test_attack_step() {
    let mut game = risk_ext::start_game(2, 3, 1, 0.0, 1, 10, 0);
    game.step(0, 1);
    // game.step(0, 1);
    assert_eq!(game.turn_idx, 0);
...
#[test]
fn test_two_attacks() {
    let mut game = risk_ext::start_game(2, 3, 1, 0.0, 2, 10, 0);
    game.step(0, 2);
    assert_eq!(game.player_idx, 0);
    game.step(0, 2);
...
fn test_skip_attack() {
    ...
    let mut game = risk_ext::start_game(2, 3, 1, 0.0, 5, 10, 0);
    game.step(0, 0);
    assert_eq!(game.turn_idx, 0);
    assert_eq!(game.player_idx, 1);
    assert_eq!(game.phase, risk_ext::Phase::Attack);
...
fn test_no_changes_after_gameover() {
    ...
    let mut game = risk_ext::start_game(2, 2, 100, 0.0, 100, 1000, 0);
    for _i in 0..100 {
        game.step(0, 1);
    }
...
fn test_game_ends_after_max_turns() {
    let mut game = risk_ext::start_game(2, 2, 1, 0.0, 1, 1, 0);
    game.step(0, 0);
    game.step(0, 0);
    assert_eq!(game.phase, risk_ext::Phase::GameOver);
...
#[rstest(per_territory, correct, case(0.0, 8), case(0.3, 8), case(0.5, 9), case(0.9, 9), case(1.0, 10), case(2.0, 12))]
fn test_reinforcements_per_territory(per_territory: f32, correct: usize) {
    let game = risk_ext::start_game(2, 4, 1, per_territory, 1, 1, 0);
    assert_eq!(game.board.territories[0].n_armies, correct);
}
